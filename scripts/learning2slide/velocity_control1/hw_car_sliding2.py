...
def setup_triangle_command_sequence():
    ...
    infinity_center = np.array([0.4, -0.3, 0.03])
    infinity_center_pose = np.zeros((6,))
    infinity_center_pose[:3] = np.array([np.pi,0.0,1.0*np.pi])
    ...
    # 3. Get Up To Speed
    throw_twist = np.zeros((6,))
    throw_twist[3:] = np.array([0.0,0.5,0])
    ccs.append(PartialStateCommand(
        name="upper_right",
        target_type = EndEffectorTarget.kTwist,
        target_value=throw_twist,
        duration=3.0,
        gripper_value=gripper_close))

    # 4. Release
    throw_twist = np.zeros((6,))
    throw_twist[3:] = np.array([0.0,0.5,0.3])
    ccs.append(PartialStateCommand(
        name="upper_right",
        target_type = EndEffectorTarget.kTwist,
        target_value=throw_twist,
...
with KinovaStationHardwareInterface(n_dof) as station:
    ...
    if run:
        ...
        if show_plots:
            ...
            plt.savefig('slide_data2/pose_data1.png', bbox_inches='tight')
            ...
            plt.savefig('slide_data2/twist_data1.png', bbox_inches='tight')
            ...
            plt.savefig('slide_data2/command_v_twists1.png', bbox_inches='tight', dpi = 100)
