...
initial_move = True                    # Move arm to the calibration position
...
p_base_cam = np.zeros((1,3))
iter_count = 0

" Main Loop "

roll = 0 # for roll in [-1,1]:
for pitch in [-1,1]:
    for yaw in [-1,1]:
        width = 0 # for width in [-1,1]:
        for depth in [-1,1]:
            for height in [-1,1]:
                ...
                # closed properly even if there is an error (e.g. KeyboardInterrupt) during execution.
                        ...
                        if initial_move:
                            ...
                            initial_move = False         # Finish the initial move
                ...
                R_base_cam = R_base_cam + X_base_cam.rotation().matrix()
                p_base_cam = p_base_cam + X_base_cam.translation()
                iter_count = iter_count + 1
                
                time.sleep(10.0) 

R_base_cam = R_base_cam/iter_count
p_base_cam = p_base_cam/iter_count
print(f"\n Final Camera Extrinsics (X_base_cam): \n {R_base_cam} \n {p_base_cam} \n")
