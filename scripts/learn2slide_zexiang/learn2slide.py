...
# closed properly even if there is an error (e.g. KeyboardInterrupt) during execution.
    ...
    target_y = 0.714
    limit_y = 0.5
    target_release_pos = None
    try: 
    # ============= Learning Phase ===============
        release_pos = [-0.2, -0.2, -0.1, -0.1, 0.0, 0.0, 0.1, 0.1, 0.2]
        terminal_y = []
        while(1):
            # Detect April Tag
            t1 = time.time()
            ...
            current_pose = np.array([1.0*np.pi, 0.0*np.pi, 1.0*np.pi, X_base_atag[0,3]-0.035, X_base_atag[1,3]-0.06, 0.10])
            ...
            if target_release_pos is None:
                cont.SetSlidingMode(release_pos = release_pos[cnt])
            else:
                cont.SetSlidingMode(release_pos = target_release_pos) 
            cnt += 1
            simulator.AdvanceTo(context.get_time() + 1)
            atag_pos = tracker.end_track()
            atag_vel = velocity_estimator(atag_pos[0], atag_pos[1])
            ...
            avg_pose = tracker.detect()
            terminal_y.append(avg_pose[1,3]) # terminal position of the car
            ...
            ax2.plot(atag_pos[0][:len(atag_vel[1])], atag_vel[1], label = "atag y velocity (esitmated)")
            ax2.legend()
            plt.draw()
            plt.pause(1)
            plt.savefig("states_"+str(cnt)+".png")
            if target_release_pos is not None:
                print("Learning to slide ends...")
                break
            # ===== Training End =====
            if cnt == len(release_pos) or limit_y - terminal_y[-1] + release_pos[cnt-1] <= release_pos[cnt]:
                target_release_pos = np.mean(np.array(release_pos[:cnt]) - np.array(terminal_y) + target_y, axis = 0)
            ax1.clear()
            ax2.clear()
        avg_pose = tracker.detect()
        print("The target release position is ", target_release_pos)
        print("The final position is ", avg_pose[1, 3])
