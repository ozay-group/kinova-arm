...
class Computer(LeafSystem):
    def __init__(self, params):
        ...
        self.computer_output_port = self.DeclareStateOutputPort("solver_feeder", self.state_index)

        # Update paddle acceleration with discrete period according to function DesignController()
        self.period = params.h
        self.DeclarePeriodicDiscreteUpdateEvent(self.period, 0, self._solver_feeder)

        # 
        self.cntr = 0
        self.x0 = np.zeros(10)
        self.simulation_duration = 0.5 #TODO: make this a parameter
        num = int(self.simulation_duration / self.period + 1)
        self.x_hist = np.zeros((num, 10))
    ...
    def _solver_feeder(self, output):
        output.set_value(self.x0)
