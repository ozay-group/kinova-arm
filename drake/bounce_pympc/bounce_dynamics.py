...
def symbolic_bounce_dynamics_restitution(stp=params.h):
    ...
    xdb_next = xdb + stp*ftf/params.m
    ydb_next = ydb + stp*fnf/params.m - stp*params.g
    tdb_next = tdb + params.r*stp*ftf/params.j

    # symbolic ball position update
    xb_next = xb + stp*xdb_next
    yb_next = yb + stp*ydb_next
    tb_next = tb + stp*tdb_next

    # symbolic floor velocity update
    xdf_next = xdf + stp*xd2f
    ydf_next = ydf + stp*yd2f

    # symbolic floor position update
    xf_next = xf + stp*xdf_next
    yf_next = yf + stp*ydf_next
...
def _dyn_test():
    ...
    times = [time_step * i for i in range(len(heights))]
    ...
    plt.ylabel("Height (m)")
    plt.legend()
    plt.savefig("ball_trajectory.png") # plt.show()
