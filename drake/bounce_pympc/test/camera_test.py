...
class Camera(LeafSystem):
    def __init__(self, params):
        ...
        self.intr_m = np.array([[self.intr.fx, 0, self.intr.ppx], [0, self.intr.fy, self.intr.ppy], [0, 0, 1]])
    ...
    def _find_location(self, f_m, intrinsic_m, extrinsic_m, depth_scale):
            frame_coordinate = np.array([f_m[0], f_m[1], f_m[2]*depth_scale + self.ball_radius])
            X_CameraBall = frame_coordinate @ intrinsic_m
            X_CameraBall = np.append(X_CameraBall,[1.0])
            # X_CameraBall = np.eye(4)
            # X_CameraBall[0:3, 0:3] = intrinsic_m
            X_B = extrinsic_m @ X_CameraBall
            return X_B
    ...
    def DoUpdate(self, context, xb):
        ...
        world_corrdinate = np.array([0, 0, 0])
        if circles is not None:
            ...
            world_corrdinate = self.extr_m @ np.append(X_CameraBall, [1])
